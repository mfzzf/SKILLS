...
## 10. 与 Claude Code / Codex / 通用 agents 客户端实时同步

Claude Code 读取 `~/.claude/skills/`，Codex 读取 `~/.codex/skills/`，其它本机 agent runtime（Cursor、自研 agent 等）读取 `~/.agents/skills/`。我们要的是**改 catalog → 三个客户端立刻看到**，而不是 copy。结论：用 **symlink**，不要 copy。
...
- 三个目标根独立处理：某一根冲突不会阻塞其它两根

### 实时同步是怎么实现的

- 你在 catalog 里 `make bump SKILL=frontend-build-2026`：子目录 commit 移动 → 三个客户端通过 symlink 立刻看到新内容，**零延迟、零 copy**。
- 你在 `~/.claude/skills/foo-skill/SKILL.md` 里改一行：因为是 symlink，等同于改 catalog 子模块本体；走正常的"在源仓改 → push → catalog bump"流程（见 §5）。
- 升级所有上游 Anthropic skill：`make bump SKILL=anthropics-skills && make push`——三个客户端同时拿到新版。
...
- `[external]` — 指向别处（你之前装的，比如 `~/.agents/skills/drawio` 老链接）
...
```bash
make link CLAUDE_ROOT=/tmp/claude-skills CODEX_ROOT=/tmp/codex-skills AGENTS_ROOT=/tmp/agents-skills
```

把 `AGENTS_ROOT=` 留空当前实现仍会把 link 放到 `/skills`（空根 + `/skills`）——如果你确实只想链 Claude+Codex，覆盖 `AGENTS_ROOT` 指向一个 throwaway 目录即可。
